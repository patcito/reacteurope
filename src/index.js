...
const Scene = React.memo(({ urls, page, setPage, setBackground }) => {
  ...
  useEffect(
    () =>
      void setInterval(() => {
        setPage(i => (i + 1) % urls.length);
        setBackground(i => (i + 1) % urls.length);
      }, 3000),
    []
  );
        ...
        {transitions.map(
          ({
            ...
            <a.mesh
              key={key}
              rotation={rotation}
              position={position.interpolate((x, y, z) => [x, y, z + index])}
            >
              <a.meshPhongMaterial
                attach="material"
                color={color}
  ...
  return (
    <div
      ...
      style={{
        ...
        transition: "transition: colorchange 5s"
      }}
    >
      <Canvas
        invalidateFrameloop
      ...
      >
        <Suspense fallback={null}>
          <Scene
            ...
            urls={["/1.svg", "/2.svg", "/4.svg", "/5.svg", "/6.svg", "/7.svg"]}
          />
        </Suspense>
      </Canvas>
      <TextContainer>
